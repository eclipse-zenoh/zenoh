...
impl Storage for MemoryStorage {
    ...
    async fn on_sample(&mut self, mut sample: Sample) -> ZResult<()> {
        trace!("on_sample for {}", sample.res_key);
        sample.ensure_timestamp();
        let timestamp = sample.timestamp.unwrap();
        match sample.kind {
            SampleKind::Put => match self.map.write().await.entry(sample.res_key.to_string()) {
                Entry::Vacant(v) => {
                    v.insert(Present {
                        sample,
                        ts: timestamp,
                ...
                Entry::Occupied(mut o) => {
                    ...
                    if old_val.ts() < &timestamp {
                        ...
                        debug!("PUT on {} dropped: out-of-date", sample.res_key);
                    }
                }
            },
            SampleKind::Delete => match self.map.write().await.entry(sample.res_key.to_string()) {
                Entry::Vacant(v) => {
                    ...
                    let cleanup_handle = self.schedule_cleanup(sample.res_key.to_string()).await;
                    v.insert(Removed {
                        ts: timestamp,
                        cleanup_handle,
                    });
                ...
                Entry::Occupied(mut o) => {
                    match o.get() {
                        ...
                        Present { sample: _, ts } => {
                            if ts < &timestamp {
                                let cleanup_handle =
                                    self.schedule_cleanup(sample.res_key.to_string()).await;
                                o.insert(Removed {
                                    ts: timestamp,
                                    cleanup_handle,
                                });
                            } else {
                                debug!("DEL on {} dropped: out-of-date", sample.res_key);
                            }
                        }
                    }
                }
            ...
            SampleKind::Patch => {
                warn!("Received PATCH for {}: not yet supported", sample.res_key);
            }
        }
        Ok(())
    }
        ...
        if !query.key_selector().as_str().contains('*') {
            ...
            for (_, stored_value) in self.map.read().await.iter() {
                if let Present { sample, ts: _ } = stored_value {
                    if resource_name::intersect(
                        query.key_selector().as_str(),
