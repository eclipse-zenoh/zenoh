...
async fn get_data(session: &zenoh::Session, key_expr: &str) -> HashMap<OwnedKeyExpr, Value> {
    let replies = session.get(key_expr)
    .res()
    .await
    .unwrap();
    ...
    while let Ok(reply) = replies.recv_async().await {
        match reply.sample {
            Ok(sample) => {
                ...
                data.insert(OwnedKeyExpr::from_str(sample.key_expr.as_str()).unwrap(), sample.value);
            }
...
async fn test_wild_card_in_order() {
    ...
    let storage = zplugin_storage_manager::StoragesPlugin::start("storage-manager", &runtime).unwrap();

    let session = zenoh::init(runtime).res().await.unwrap();

    put_data(&session, "*", "1", Timestamp::from_str("2022-01-17T10:42:10.418555997Z/BC779A06D7E049BD88C3FF3DB0C17FCC").unwrap()).await;
