...
pub const QUERY_TARGET_DEFAULT: QueryTarget = QueryTarget { storage: Target::BestMatching, eval: Target::BestMatching };

impl Default for &QueryTarget {
    fn default() -> Self { &QUERY_TARGET_DEFAULT }
}
...
fn criterion_benchmark(c: &mut Criterion) {
  let mut sum1 = 0u64;
  c.bench_function("pass_by_ref_option None", |b| b.iter(|| {
    pass_by_ref_option(&None, &mut sum1);
  }));

  let mut sum2 = 0u64;
  let v = Some(QUERY_TARGET_DEFAULT);
  c.bench_function("pass_by_ref_option Some()", |b| b.iter(|| {
    pass_by_ref_option(&v, &mut sum2);
  }));

  let mut sum3 = 0u64;
  c.bench_function("pass_by_option None", |b| b.iter(|| {
    pass_by_option(None, &mut sum3);
  }));

  let mut sum4 = 0u64;
  c.bench_function("pass_by_option Some()", |b| b.iter(|| {
    pass_by_option(Some(QUERY_TARGET_DEFAULT), &mut sum4);
  }));

  let mut sum5 = 0u64;
  c.bench_function("pass_by_ref default", |b| b.iter(|| {
    pass_by_ref(&QueryTarget::default(), &mut sum5);
  }));

  let mut sum6 = 0u64;
  c.bench_function("pass_by_ref QUERY_TARGET_DEFAULT", |b| b.iter(|| {
    pass_by_ref(&QUERY_TARGET_DEFAULT, &mut sum6);
  }));

  let mut sum7 = 0u64;
  c.bench_function("pass_by_val default", |b| b.iter(|| {
