...
async fn main() {
    ...
    println!("Creating PointToPointServer on {}", key_expr);
    let mut server = session.point_to_point_server(key_expr).await.unwrap();
...
fn parse_args() -> (Config, String, String) {
    let args = App::new("zenoh-ext point to point responder example")
        .arg(
        ...
        .arg(
            Arg::from_usage("-k, --key=[KEYEXPR]        'The key expression to create a point to point server on.'")
                .default_value("/demo/example/ptp-server"),
        )
        .arg(
            Arg::from_usage("-v, --value=[VALUE]      'The value to send in replies.'")
                .default_value("Hello from responder!"),
        )
        .arg(Arg::from_usage(
