...
#[derive(Clone, PartialEq, Eq, Hash, Default)]
pub struct OrderedProperties<'s>(Parameters<'s>);
...
impl<'s> From<Parameters<'s>> for OrderedProperties<'s> {
...
impl<'s> From<&'s str> for OrderedProperties<'s> {
    fn from(value: &'s str) -> Self {
        Self::from(Parameters::from(value))
    }
}
...
{
    fn from_iter<T: IntoIterator<Item = (&'s K, &'s V)>>(iter: T) -> Self {
        Self::from(Parameters::from_iter(iter))
    }
}
