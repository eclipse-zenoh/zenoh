...
impl ParametersView {
        ...
        I: Iterator<Item = (&'s str, &'s str)>,
    {
        ...
        ParametersView::from_iter_into(iter, &mut into);
        into
    }
    ...
    pub fn insert<'s>(s: &'s str, k: &'s str, v: &'s str) -> (String, Option<&'s str>) {
        let (iter, item) = ParametersView::_insert(ParametersView::iter(s), k, v);
        (ParametersView::from_iter(iter), item)
    }
    ...
    {
        let mut into = String::new();
        ParametersView::concat_into(iter, &mut into);
        into
    }
