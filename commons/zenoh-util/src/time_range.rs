...
use std::{time::{Duration, SystemTime}, str::FromStr, ops::Add};
use zenoh_core::{bail, zresult::ZError, zerror};
...
impl FromStr for TimeRange {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some((start, end)) = s.split_once("..") {
               Ok(TimeRange(parse_time_bound(start, inclusive_start)?, parse_time_bound(end, inclusive_end)?))
            ...
            let end_bound = match &start_bound {
                ...
                TimeBound::Unbounded =>
                    bail!(r#"Invalid TimeRange (';' must separate a time and a duration)"): {}"#, s)
            };
            Ok(TimeRange(start_bound, end_bound))
        } else {
            bail!(r#"Invalid TimeRange (must contain ".." or ";" as separator)"): {}"#, s)
        }
    }
}
...
impl FromStr for TimeExpr {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.starts_with("now(") && s.ends_with(')') {
            ...
            parse_rfc3339_weak(s).map_err(|e| zerror!(e)).map(TimeExpr::Fixed)
        }
...
impl Add<f64> for TimeExpr {
    ...
    fn add(self, duration: f64) -> Self {
        match self {
            ...
            Self::Now{ offset_secs } => Self::Now { offset_secs: offset_secs + duration },
        }
    }
}
...
impl <'a>  Add<f64> for &'a TimeExpr {
    ...
    fn add(self, duration: f64) -> TimeExpr {
        match self {
            ...
            TimeExpr::Now{ offset_secs } => TimeExpr::Now { offset_secs: offset_secs + duration },
        }
    }
}
...
fn parse_duration(s: &str) -> Result<f64, ZError> {
    if s.is_empty() {
        bail!(r#"Invalid duration: "" (expected format: <f64> (in seconds) or <f64><unit>. Accepted units: u, ms, s, m, h, d or w.)"#);
    }
...
mod tests {
    ...
    fn test_parse_time_range() {
        use TimeBound::*;
        use TimeExpr::*;
        assert_eq!("[..]".parse::<TimeRange>().unwrap(),
    ...
    fn test_parse_time_expr() {
        assert_eq!("2022-06-30T01:02:03.226942997Z".parse::<TimeExpr>().unwrap(),
            TimeExpr::Fixed(humantime::parse_rfc3339("2022-06-30T01:02:03.226942997Z").unwrap()));
        assert_eq!("2022-06-30T01:02:03Z".parse::<TimeExpr>().unwrap(),
            TimeExpr::Fixed(humantime::parse_rfc3339("2022-06-30T01:02:03Z").unwrap()));
        assert_eq!("2022-06-30T01:02:03".parse::<TimeExpr>().unwrap(),
            TimeExpr::Fixed(humantime::parse_rfc3339("2022-06-30T01:02:03Z").unwrap()));
        assert_eq!("2022-06-30 01:02:03Z".parse::<TimeExpr>().unwrap(),
            TimeExpr::Fixed(humantime::parse_rfc3339("2022-06-30T01:02:03Z").unwrap()));
        assert_eq!("now()".parse::<TimeExpr>().unwrap(),
            TimeExpr::Now{ offset_secs: 0.0});
        assert_eq!("now(123.45)".parse::<TimeExpr>().unwrap(),
            TimeExpr::Now{ offset_secs: 123.45});
        assert_eq!("now(1h)".parse::<TimeExpr>().unwrap(),
            TimeExpr::Now{ offset_secs: 3600.0});
        assert_eq!("now(-1h)".parse::<TimeExpr>().unwrap(),
            TimeExpr::Now{ offset_secs: -3600.0});
