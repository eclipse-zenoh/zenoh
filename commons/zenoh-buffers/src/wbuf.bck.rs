...
use crate::writer::{BacktrackableWriter, DidntWrite, HasWriter, Writer};
...
impl Writer for WBufWriter {
    fn write(&mut self, bytes: &[u8]) -> Result<usize, DidntWrite> {
        Writer::write(&mut self.inner, bytes)
    }
    fn write_u8(&mut self, byte: u8) -> Result<(), DidntWrite> {
        Writer::write_u8(&mut self.inner, byte)
    }
    fn write_zslice(&mut self, slice: ZSlice) -> Result<(), DidntWrite> {
        Writer::write_zslice(&mut self.inner, slice)
    }
}
...
impl BacktrackableWriter for WBufWriter {
    ...
    fn rewind(&mut self, _mark: Self::Mark) -> bool {
...
mod tests {
    ...
    fn wbuf_noncontiguous_mark_reset() {
        ...
        assert!(buf.write_u8(0).is_ok());
        assert!(buf.write_u8(1).is_ok());
        dbg!(&buf);
        buf.rewind(());
        assert!(vec_vec_is_empty!(buf));
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![0_u8, 1])).is_ok());
        buf.rewind(());
        assert!(vec_vec_is_empty!(buf));
        ...
        assert!(buf.write(&[3, 4]).is_ok());
        assert!(buf.write_zslice(ZSlice::from(vec![5_u8, 6])).is_ok());
        assert!(buf.write_u8(7).is_ok());
        assert!(buf.write_zslice(ZSlice::from(vec![8_u8, 9])).is_ok());
        buf.rewind(());
        assert_eq!(to_vec_vec!(buf), [[0, 1, 2]]);
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![8_u8, 9])).is_ok());
        buf.mark();
        assert!(buf.write_zslice(ZSlice::from(vec![10_u8, 11])).is_ok());
        buf.rewind(());
        assert_eq!(
            to_vec_vec!(buf),
