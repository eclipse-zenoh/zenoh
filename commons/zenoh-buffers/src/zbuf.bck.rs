...
mod tests {
    ...
    fn test_zbuf() {
        ...
        for i in 0..buf1_reader.remaining() - 1 {
            assert_eq!(i as u8, buf1_reader.read_u8().unwrap());
        }
        ...
        for i in 0..buf2.len() - 1 {
            assert_eq!(i as u8, buf2_reader.read_u8().unwrap());
        }
        ...
        for i in 0..buf3.len() - 1 {
            assert_eq!(i as u8, buf3.read_u8().unwrap());
        }

        // test read_into_zbuf
        buf1_reader.reset();
        println!("[09] {:?}", buf1_reader);
        let _ = buf1_reader.read_u8();
