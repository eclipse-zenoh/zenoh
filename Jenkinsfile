pipeline {
  ...
  stages {
    ...
    stage('[MacMini] manylinux x64 build') {
      ...
      steps {
        sh '''
        docker run --init -it --rm -v $(pwd):/workdir -w /workdir adlinktech/manylinux2010-x64-rust-nightly cargo build --release --all-targets --target-dir=target/manylinux2010-x64
        '''
      }
    }
    ...
    stage('[MacMini] manylinux2010 i686 build') {
      ...
      steps {
        sh '''
        docker run --init -it --rm -v $(pwd):/workdir -w /workdir adlinktech/manylinux2010-i686-rust-nightly cargo build --release --all-targets --target-dir=target/manylinux2010-i686
        '''
      }
    }
