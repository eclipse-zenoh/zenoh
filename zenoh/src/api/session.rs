...
impl SessionInner {
    ...
    fn register_callback_drop_notifier<T>(
        ...
        T: CallbackParameter,
    {
        let n = self.callbacks_drop_sync_group.notifier();
        ...
        F: FnOnce() -> ZResult<()> + Clone + Send + Sync + 'static,
    {
        ...
        callback.set_on_drop({
            let session_notifier = self.callbacks_drop_sync_group.notifier();
            move || {
...
impl Closee for Arc<SessionInner> {
    ...
    #[allow(unused_variables)] // SessionCloseArgs are only requred for wait until callback execution ends under unstable
    async fn close_inner(&self, close_args: SessionCloseArgs) {
        ...
        if close_args.wait_until_callback_execution_ends {
            self.callbacks_drop_sync_group.wait_async().await;
        }
    }
}
