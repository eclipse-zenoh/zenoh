...
impl HatBaseTrait for HatCode {
    ...
    fn close_face(
        ...
        for (_, (mut res, c)) in hat_face.remote_qabls.drain() {
            get_mut_unchecked(&mut res).session_ctxs.remove(&face.id);
            undeclare_simple_queryable(&mut wtables, &mut face_clone, &mut res, c, send_declare);
...
struct HatFace {
    ...
    remote_qabls: HashMap<QueryableId, (Arc<Resource>, bool)>,
}
