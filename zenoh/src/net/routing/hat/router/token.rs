...
fn register_router_token(
    ...
    if !res_hat!(res).router_tokens.contains(&router) {
        ...
        propagate_sourced_token(tables, res, Some(face), &router);
    }
...
fn propagate_forget_simple_token_to_peers(
    ...
    if res_hat!(res).router_tokens.len() == 1 && res_hat!(res).router_tokens.contains(&tables.zid) {
            ...
            .collect::<Vec<Arc<FaceState>>>()
        {
            if face.whatami == WhatAmI::Peer
                ...
                && !res.session_ctxs.values().any(|s| {
                    face.zid != s.face.zid
                        && s.token
                        && (s.face.whatami == WhatAmI::Client || s.face.whatami == WhatAmI::Peer)
                })
            {
                if let Some(id) = face_hat_mut!(&mut face).local_tokens.remove(res) {
                    send_declare(
...
fn undeclare_router_token(
    ...
    if res_hat!(res).router_tokens.contains(router) {
        ...
        propagate_forget_sourced_token(tables, res, face, router);
    }
}
    ...
    for (tree_sid, tree_clildren) in new_clildren.iter().enumerate() {
        if !tree_clildren.is_empty() {
            ...
            if net.graph.contains_node(tree_idx) {
                ...
                let tokens_res = &hat!(tables).router_tokens;
