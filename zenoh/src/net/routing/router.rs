...
impl Router {
    pub fn new(zid: ZenohIdProto, hlc: Option<Arc<HLC>>, config: &Config) -> ZResult<Self> {
        let hats = [
            (Bound::eastwest0(), config.mode().unwrap_or_default()),
            (
                Bound::south0(),
                config.gateway.south.mode().unwrap_or_default(),
            ),
        ];
            ...
            tables: Arc::new(TablesLock {
                tables: RwLock::new(Tables {
                    data: TablesData::new(
                        ...
                        hats.iter()
                            .copied()
                            .map(|(b, wai)| (b, tables::HatData::new(wai)))
                            .collect(),
                    )?,
                    hat: hats
                        .iter()
    ...
    pub fn init_hats(&mut self, runtime: Runtime) -> ZResult<()> {
        ...
        for (_, hat) in tables.hat.iter_mut() {
    ...
    pub(crate) fn new_primitives(
        ...
        let newface = tables.data.hat[bound]
            .faces
            .entry(fid)
            .or_insert_with(|| {
                Arc::new(
                    FaceStateBuilder::new(
                        ...
                        bound,
                        primitives.clone(),
                        tables.hat.map(|hat| hat.new_face()),
                    )
    ...
    pub fn new_transport_unicast(
        ...
        let mut declares = vec![];
        tables.hat[bound].new_transport_unicast_face(
            &mut tables.data,
            &self.tables,
            &mut face,
            &transport,
    ...
    pub fn new_transport_multicast(
        ...
        tables.data.hat[bound].mcast_groups.push(face);

        tables.data.hat[bound].disable_all_routes();
        Ok(())
    }
    ...
    pub fn new_peer_multicast(
        ...
        tables.data.hat[bound].mcast_faces.push(face_state.clone());
