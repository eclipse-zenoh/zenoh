...
impl Tables {
    ...
    pub(crate) fn schedule_compute_trees(&mut self, tables_ref: Arc<RwLock<Tables>>) {
        log::trace!("Schedule computations");
        if self.routers_trees_task.is_none() {
            let task = Some(async_std::task::spawn(async move {
                ...
                pubsub_tree_change(&mut tables, &new_childs);
                queries_tree_change(&mut tables, &new_childs);

                log::trace!("Computations completed");
                tables.routers_trees_task = None;
            }));
            self.routers_trees_task = task;
        }
    }
}
...
impl Router {
    ...
    pub fn new_transport_unicast(
        ...
        if tables.whatami == WhatAmI::Router && whatami == WhatAmI::Router {
            tables.schedule_compute_trees(self.tables.clone());
        }
        Ok(handler)
    }
}
            ...
            ZenohBody::LinkStateList(list) => {
                if let Ok(pid) = self.transport.get_pid() {
                    ...
                    if tables.whatami == WhatAmI::Router && whatami == WhatAmI::Router {
                        ...
                        {
                            pubsub_remove_node(&mut tables, &removed_node.pid);
                            queries_remove_node(&mut tables, &removed_node.pid);
                        }
        ...
        match (self.transport.get_pid(), self.transport.get_whatami()) {
            (Ok(pid), Ok(whatami)) => {
                ...
                if tables.whatami == WhatAmI::Router && whatami == WhatAmI::Router {
                    ...
                    tables.schedule_compute_trees(tables_ref.clone());
                }
            }
