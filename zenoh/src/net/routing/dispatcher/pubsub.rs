...
use crate::net::routing::{
    dispatcher::{
        face::{Face, FaceId},
        gateway::{Bound, BoundKind},
    },
    hat::{InterestProfile, SendDeclare},
...
impl Face {
    ...
    pub(crate) fn declare_subscription(
            ...
            .cloned()
        {
            Some(mut prefix) => {
                ...
                tables.hat[self.state.bound].handle_owned_subscription(
                    &mut tables.data,
                ...
                for (_, hat) in tables
                    .hat
                    .eastwest_iter_mut()
                    .filter(|(b, _)| *b != self.state.bound)
                {
                    hat.handle_shared_subscription(
                        &mut tables.data,
                        &mut self.state.clone(),
                        id,
                        &mut res,
                ...
                let south_profile = match self.state.bound.kind {
                    BoundKind::Eastwest => InterestProfile::Pull,
                    BoundKind::South => InterestProfile::Push,
                };
                ...
                for bound in tables.hat.bounds() {
    ...
    pub(crate) fn undeclare_subscription(
        ...
        for (bound, hat) in tables.hat.iter_mut() {
            // FIXME(fuzzypixelz): gateway interest management
            if let Some(mut res) = hat.undeclare_subscription(
                &mut tables.data,
                &mut self.state.clone(),
                id,
                res.clone(),
...
fn get_data_route(
    ...
    let local_context = tables.hat[bound].map_routing_context(&tables.data, face, routing_context);
...
pub fn route_data(
    ...
    for (bound, hat) in rtables.hat.iter() {
    ...
    let send_push = |outface: &FaceState, msg: &mut Push, reliability: Reliability| {
        #[cfg(feature = "stats")]
        if !admin {
