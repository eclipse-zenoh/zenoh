...
pub fn route_data(
    ...
    let mut dirs = SmallVec::<Direction, 2>::new();
    ...
    for (bound, hat) in rtables.hat.iter() {
        if hat.ingress_filter(&rtables.data, face, &mut expr) {
            ...
            tracing::trace!(?bound, ?route);

            dirs.extend(
                route
                    .values()
                    .filter(|dir| hat.egress_filter(&rtables.data, face, &dir.dst_face, &mut expr))
                    .cloned(),
            );
        }
    }
    ...
    if let Some((first, last)) = dirs.pop().map(|last| (last, dirs)) {
        treat_timestamp!(
            &rtables.data.hlc,
            msg.payload,
        ...
        msg.wire_expr = first.wire_expr.clone();
        ...
        send_push(&first.dst_face, msg, reliability);
    }
}
