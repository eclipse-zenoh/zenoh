            ...
            let (mut res, mut wtables) =
                if res.as_ref().map(|r| r.context.is_some()).unwrap_or(false) {
                    ...
                    let mut matches = keyexpr::new(fullexpr.as_str())
                        .map(|ke| Resource::get_matches(&rtables, ke, Resource::QAB))
                        .unwrap_or_default();
                    drop(rtables);
                    ...
                    Resource::match_resource(&wtables, &mut res, matches, Resource::QAB);
                    (res, wtables)
                };
            get_mut_unchecked::<Resource>(&mut res).flag(Resource::QAB);
...
pub(crate) fn undeclare_queryable(
        ...
        hat_code.undeclare_queryable(&mut wtables, face, id, res, node_id, send_declare)
    {
        ...
        get_mut_unchecked::<Resource>(&mut res).unflag(Resource::QAB);
        disable_matches_query_routes(&mut wtables, &mut res);
...
pub(crate) fn disable_matches_query_routes(tables: &mut Tables, res: &mut Arc<Resource>) {
    if res.context.is_some() {
        ...
        for match_ in Resource::get_matches_for(tables, res, Resource::QAB).iter() {
