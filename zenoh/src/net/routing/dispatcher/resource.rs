...
pub(crate) struct FaceContext {
    ...
    // REVIEW(fuzzypixelz): added because e.g. peer/peer bounds needs separate
    // sub/qabl/token ctxs for eastwest-bound and south-bound clients
    pub(crate) hat: BoundMap<HatFaceContext>,
}

#[derive(Debug, Clone, Default)]
pub(crate) struct HatFaceContext {
...
impl FaceContext {
    pub(crate) fn new(tables: &TablesData, face: Arc<FaceState>) -> Self {
        Self {
            face,
            local_expr_id: None,
            remote_expr_id: None,
            hat: tables
                .hat
                .bounds()
                .map(|b| (b, HatFaceContext::default()))
                .collect(),
...
pub(crate) struct ResourceContext {
    ...
    pub(crate) hat: BoundMap<HatResourceContext>,
}
        ...
        Some(mut prefix) => match face.remote_mappings.get(&expr_id) {
            ...
            None => {
                ...
                let ctx = get_mut_unchecked(&mut res)
                    .face_ctxs
                    .entry(face.id)
                    .or_insert_with(|| Arc::new(FaceContext::new(&wtables.data, face.clone())));
                ...
                for bound in wtables.hat.bounds() {
