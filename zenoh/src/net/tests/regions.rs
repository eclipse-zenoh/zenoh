...
fn test_client_to_client_query_route_computation() {
    for mode in [WhatAmI::Client, WhatAmI::Peer, WhatAmI::Router] {
        ...
        let buf = Arc::new(RequestBuffer::default());
        let dst_face = router.new_face(buf.clone(), Bound::north(), WhatAmI::Client, false);
        ...
        src_face.route_query(&mut msg(1));
        let buf_guard = buf.0.lock().unwrap();
        assert_eq!(&*buf_guard, &[msg(1)]);
    }
