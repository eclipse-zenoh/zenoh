...
pub const ZNS_QUEUE_NUM: usize = 3;

// Queue priorities
pub const ZNS_QUEUE_PRIO_CTRL: usize = 0;
pub const ZNS_QUEUE_PRIO_RETX: usize = 1;
pub const ZNS_QUEUE_PRIO_DATA: usize = 2;

zconfigurable! {
    // Default link lease in milliseconds: 10 seconds
    pub static ref ZNS_LEASE: ZInt = 10_000;
    ...
    //       session lease.
    pub static ref ZNS_KEEP_ALIVE: ZInt = 2_500;
    ...
    pub static ref ZNS_SEQ_NUM_RESOLUTION: ZInt = 268_435_456;
    ...
    // an MTU smaller than the ZNS_BATCH_SIZE, the batch size will be automatically set to the
    // transport link MTU to avoid any transmission problems on the network.
    pub static ref ZNS_BATCH_SIZE: usize = 65_537;

    // Default timeout when opening a session in milliseconds
    pub static ref ZNS_OPEN_TIMEOUT: u64 = 10_000;

    // Default maximum number of pending sessions being opened with the host
    pub static ref ZNS_OPEN_MAX_CONCURRENT: usize = 1_024;
    ...
    // - The amount of memory being allocated for each queue is then QUEUE_SIZE_XXX * ZNS_BATCH_SIZE.
    //   In case the transport link MTU is smaller than the ZNS_BATCH_SIZE, then amount of memory being
    //   allocated for each queue is QUEUE_SIZE_XXX * LINK_MTU.
    pub static ref ZNS_QUEUE_SIZE_CTRL: usize = 1;
    pub static ref ZNS_QUEUE_SIZE_RETX: usize = 1;
    pub static ref ZNS_QUEUE_SIZE_DATA: usize = 4;

    // The default backoff time in nanoseconds to allow the batching to potentially progress
    pub static ref ZNS_QUEUE_PULL_BACKOFF: u64 = 100;

    // The total size of buffers allocated at RX side per link. Default 16MB.
    pub static ref ZNS_RX_BUFF_SIZE: usize = 16_777_216;
}
