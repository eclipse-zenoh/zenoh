...
mod tests {
    ...
    fn wbuf_contiguous_write_zslice() {
        let mut buf = WBuf::new(6, true);
        assert!(buf.write_zslice(ZSlice::from(vec![0u8, 1, 2])));
        assert_eq!(buf.len(), 3);
        assert_eq!(buf.capacity(), 6);
        assert_eq!(to_vec_vec!(buf), [[0, 1, 2]]);

        assert!(buf.write_zslice(ZSlice::from(vec![3u8, 4])));
        assert_eq!(buf.len(), 5);
        assert_eq!(buf.capacity(), 6);
        assert_eq!(to_vec_vec!(buf), [[0, 1, 2, 3, 4]]);

        assert!(!buf.write_zslice(ZSlice::from(vec![5u8, 6])));
        assert_eq!(buf.len(), 5);
        assert_eq!(buf.capacity(), 6);
        assert_eq!(to_vec_vec!(buf), [[0, 1, 2, 3, 4]]);

        assert!(buf.write_zslice(ZSlice::from(vec![5u8])));
        assert_eq!(buf.len(), 6);
        assert_eq!(buf.capacity(), 6);
        assert_eq!(to_vec_vec!(buf), [[0, 1, 2, 3, 4, 5]]);
    }
    ...
    fn wbuf_contiguous_copy_into_slice() {
        let mut buf = WBuf::new(6, true);
        assert!(buf.write_zslice(ZSlice::from(vec![0u8, 1, 2, 3, 4, 5])));
    ...
    fn wbuf_noncontiguous_write_zslice() {
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![7u8])));
        assert_eq!(buf.len(), 8);
        assert_eq!(buf.capacity(), 6);
        assert_eq!(
            to_vec_vec!(buf),
    ...
    fn wbuf_noncontiguous_write_mixed() {
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![2u8, 3, 4])));
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![9u8, 10, 11])));
        assert_eq!(
            to_vec_vec!(buf),
    ...
    fn wbuf_noncontiguous_get_first_slice_mut() {
        ...
        assert!(buf.write_bytes(&[1, 2, 3, 4, 5]));
        // add an ZSlice
        assert!(buf.write_zslice(ZSlice::from(vec![6u8, 7, 8, 9, 10])));
    ...
    fn wbuf_noncontiguous_mark_reset() {
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![0u8, 1])));
        buf.revert();
        assert!(vec_vec_is_empty!(buf));
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![5u8, 6])));
        assert!(buf.write(7));
        assert!(buf.write_zslice(ZSlice::from(vec![8u8, 9])));
        buf.revert();
        assert_eq!(to_vec_vec!(buf), [[0, 1, 2]]);
        ...
        assert!(buf.write_zslice(ZSlice::from(vec![8u8, 9])));
        buf.mark();
        assert!(buf.write_zslice(ZSlice::from(vec![10u8, 11])));
        buf.revert();
        assert_eq!(
            to_vec_vec!(buf),
