...
use zenoh_util::handler::{KeyedHandler, KeyedHandlers};
...
impl ConfigValidator for AdminSpace {
    fn check_config(
        ...
        #[cfg(feature = "plugins")]
        {
            let plugins_mgr = self.runtime.plugins_manager();
...
impl AdminSpace {
    ...
    pub async fn start(runtime: &Runtime, version: String) {
        ...
        let mut handlers: KeyedHandlers<Query> = KeyedHandlers::default();
        handlers.insert(
            root_key.clone(),
            LocalDataHandler {
        ...
        #[cfg(all(feature = "plugins", feature = "runtime_plugins"))]
        {
            let cfg_rx = admin.runtime.state.config.subscribe();
                ...
                async move {
                    while let Ok(change) = cfg_rx.recv_async().await {
                        ...
                        let requested_plugins = {
                            let cfg_guard = admin.runtime.state.config.lock();
                        ...
                        let mut plugins_mgr = admin.runtime.plugins_manager();
                        for diff in diffs {
                            match diff {
...
impl Primitives for AdminSpace {
    ...
    fn send_push(&self, msg: &mut Push, _reliability: Reliability) {
        trace!("recv Push {:?}", msg);
        {
            let conf = &self.runtime.state.config.lock().0;
        ...
        if let Some(key) = msg.wire_expr.as_str().strip_prefix(&format!(
            "@/{}/{}/config/",
            self.runtime.state.zid, self.runtime.state.whatami,
        )) {
            match &msg.payload {
                PushBody::Put(put) => match std::str::from_utf8(&put.payload.contiguous()) {
                    Ok(json) => {
                        tracing::trace!(
                            ...
                            self.runtime.state.zid,
                            self.runtime.state.whatami,
                            key,
                            json
                        );
                        if let Err(e) = self.runtime.state.config.insert_json5(key, json) {
                            error!(
                                "Error inserting conf value @/{}/{}/config/{} : {} - {}",
                                self.runtime.state.zid, self.runtime.state.whatami, key, json, e
                            );
                        }
                    }
                    Err(e) => error!(
                        "Received non utf8 conf value on @/{}/{}/config/{} : {}",
                        self.runtime.state.zid, self.runtime.state.whatami, key, e
                    ),
                },
                PushBody::Del(_) => {
                    tracing::trace!(
                    ...
                    if let Err(e) = self.runtime.state.config.remove(key) {
...
impl KeyedHandler<Query> for LocalDataHandler {
...
impl KeyedHandler<Query> for MetricsHandler {
    fn handle(&self, query: &Query) {
        let reply_key: OwnedKeyExpr = format!(
            "@/{}/{}/metrics",
            self.runtime.state.zid, self.runtime.state.whatami
...
struct RoutersLinkstateDataHandler {
    runtime: Runtime,
}
impl KeyedHandler<Query> for RoutersLinkstateDataHandler {
    fn handle(&self, query: &Query) {
        let reply_key: OwnedKeyExpr = format!(
            "@/{}/{}/linkstate/routers",
            self.runtime.state.zid, self.runtime.state.whatami
        ...
        let tables = zread!(self.runtime.state.router.tables.tables);
...
struct PeersLinkstateDataHandler {
    runtime: Runtime,
}
impl KeyedHandler<Query> for PeersLinkstateDataHandler {
    fn handle(&self, query: &Query) {
        let reply_key: OwnedKeyExpr = format!(
            "@/{}/{}/linkstate/peers",
            self.runtime.state.zid, self.runtime.state.whatami
...
struct SubscribersDataHandler {
    runtime: Runtime,
}
impl KeyedHandler<Query> for SubscribersDataHandler {
    fn handle(&self, query: &Query) {
        let tables = zread!(self.runtime.state.router.tables.tables);
...
struct PublishersDataHandler {
    runtime: Runtime,
}
impl KeyedHandler<Query> for PublishersDataHandler {
    fn handle(&self, query: &Query) {
        let tables = zread!(self.runtime.state.router.tables.tables);
...
struct QueryablesDataHandler {
    runtime: Runtime,
}
impl KeyedHandler<Query> for QueryablesDataHandler {
    fn handle(&self, query: &Query) {
        let tables = zread!(self.runtime.state.router.tables.tables);
...
impl KeyedHandler<Query> for QueriersDataHandler {
    fn handle(&self, query: &Query) {
        let tables = zread!(self.runtime.state.router.tables.tables);
...
#[cfg(feature = "plugins")]
struct PluginsDataHandler {
    runtime: Runtime,
}
#[cfg(feature = "plugins")]
impl KeyedHandler<Query> for PluginsDataHandler {
    fn handle(&self, query: &Query) {
        let guard = self.runtime.plugins_manager();
        let root_key = format!(
            "@/{}/{}/plugins",
            &self.runtime.state.zid, self.runtime.state.whatami
        );
...
#[cfg(feature = "plugins")]
struct PluginsStatusHandler {
    runtime: Runtime,
}
#[cfg(feature = "plugins")]
impl KeyedHandler<Query> for PluginsStatusHandler {
    fn handle(&self, query: &Query) {
