...
impl AdminSpace {
    ...
    pub async fn start(runtime: &Runtime, version: String) {
        ...
        let primitives = runtime
            .state
            .router
            .new_primitives(admin.clone(), Bound::south0()); // REVIEW(fuzzypixelz): should adminspace be south-bound or eastwest-bound?
...
fn routers_linkstate_data(_context: &AdminContext, _query: Query) {
    ...
    // if let Err(e) = query
    //     .reply(reply_key, rtables.hats.info(WhatAmI::Router))
    //     .encoding(Encoding::TEXT_PLAIN)
    //     .wait()
    // {
    //     tracing::error!("Error sending AdminSpace reply: {:?}", e);
...
fn subscribers_data(_context: &AdminContext, _query: Query) {
    ...
    // let tables = &context.runtime.state.router.tables;
    // let rtables = zread!(tables.tables);
    // for sub in rtables.hats.get_subscriptions(&rtables.data) {
...
fn publishers_data(_context: &AdminContext, _query: Query) {
    ...
    // let tables = &context.runtime.state.router.tables;
    // let rtables = zread!(tables.tables);
    // for sub in rtables.hats.get_publications(&rtables.data) {
    //     let key = KeyExpr::try_from(format!(
    //         "@/{}/{}/publisher/{}",
    //         context.runtime.state.zid,
    //         context.runtime.state.whatami,
...
fn queryables_data(_context: &AdminContext, _query: Query) {
    ...
    // let tables = &context.runtime.state.router.tables;
    // let rtables = zread!(tables.tables);
    // for qabl in rtables.hats.get_queryables(&rtables.data) {
    //     let key = KeyExpr::try_from(format!(
    //         "@/{}/{}/queryable/{}",
    //         context.runtime.state.zid,
    //         context.runtime.state.whatami,
...
fn queriers_data(_context: &AdminContext, _query: Query) {
    ...
    // let tables = &context.runtime.state.router.tables;
    // let rtables = zread!(tables.tables);
    // for sub in rtables.hats.get_queriers(&rtables.data) {
    //     let key = KeyExpr::try_from(format!(
    //         "@/{}/{}/querier/{}",
    //         context.runtime.state.zid,
    //         context.runtime.state.whatami,
...
fn route_successor(_context: &AdminContext, _query: Query) {
    ...
    //         if let Some(successor) = rtables.hats.route_successor(src_zid, dst_zid) {
    ...
    // let successors = rtables.hats.route_successors();
    // drop(rtables);
