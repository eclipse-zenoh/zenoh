...
use crate::sealed::net::{
    primitives::Primitives, routing::dispatcher::face::Face, runtime::Runtime,
};
use std::{
    collections::HashMap,
...
impl Session {
    ...
    pub(in crate::sealed) fn declare_subscriber_inner(
        ...
        let declared_sub = origin != Locality::SessionLocal
            && !key_expr
                .as_str()
                .starts_with(super::liveliness::PREFIX_LIVELINESS);
    ...
    pub(in crate::sealed) fn declare_liveliness_inner(
        ...
        let id = self.runtime.next_id();
        let key_expr = KeyExpr::from(*super::liveliness::KE_PREFIX_LIVELINESS / key_expr);
    ...
    pub(in crate::sealed) fn matching_status(
        ...
        use crate::sealed::net::routing::dispatcher::tables::RoutingExpr;
        let router = self.runtime.router();
        let tables = zread!(router.tables.tables);
        let res = crate::sealed::net::routing::dispatcher::resource::Resource::get_resource(
            &tables.root_res,
            key_expr.as_str(),
        );

        let route = crate::sealed::net::routing::dispatcher::pubsub::get_local_data_route(
            &tables,
            &res,
...
impl crate::sealed::net::primitives::EPrimitives for Session {
    #[inline]
    fn send_declare(&self, ctx: crate::sealed::net::routing::RoutingContext<Declare>) {
    ...
    #[inline]
    fn send_request(&self, ctx: crate::sealed::net::routing::RoutingContext<Request>) {
        (self as &dyn Primitives).send_request(ctx.msg)
    }

    #[inline]
    fn send_response(&self, ctx: crate::sealed::net::routing::RoutingContext<Response>) {
        (self as &dyn Primitives).send_response(ctx.msg)
    }

    #[inline]
    fn send_response_final(&self, ctx: crate::sealed::net::routing::RoutingContext<ResponseFinal>) {
