...
async fn main() {
    ...
    let mut get_stream: GetRequestStream = session
        .register_queryable(&path.clone().to_string().into())
        .await
        .unwrap()
        .into();
    while let Some(get_request) = get_stream.next().await {
        ...
        if name.starts_with('/') {
            ...
            match DataReceiver::from(session.get(&name.clone().into()).await.unwrap())
                .next()
                .await
            {
                Some(Data {
