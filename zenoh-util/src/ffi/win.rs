...
pub fn sockaddr_to_addr(socket_addr: ws2def::SOCKET_ADDRESS) -> io::Result<SocketAddr> {
    unsafe {
        match socket_addr.lpSockaddr.as_ref().unwrap().sa_family as i32 {
            ws2def::AF_INET => {
                assert!(socket_addr.iSockaddrLength as usize >= mem::size_of::<ws2def::SOCKADDR_IN>());
                let socket_addr: &ws2def::SOCKADDR_IN =
                    &*(socket_addr.lpSockaddr.as_ref().unwrap()
            ...
            ws2def::AF_INET6 => {
                assert!(socket_addr.iSockaddrLength as usize >= mem::size_of::<ws2ipdef::SOCKADDR_IN6>());
