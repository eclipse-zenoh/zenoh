...
mod tests {
    ...
    async fn transport_simultaneous(endpoint01: Vec<EndPoint>, endpoint02: Vec<EndPoint>) {
        ...
        let peer01_task = task::spawn(async move {
            ...
            // Resolve for the links to be properly established
            ztimeout!(async {
            ...
            // Resolve for the messages to arrive to peer 01
            ztimeout!(async {
                let mut check = 0;
        ...
        let peer02_task = task::spawn(async move {
            ...
            // Resolve for the messages to arrive to peer 02
            ztimeout!(async {
                let mut check = 0;
                while check != MSG_COUNT {
                    task::sleep(SLEEP).await;
        ...
        println!("[Simultaneous] => Resolveing for peer01 and peer02 tasks...");
        peer01_task.join(peer02_task).await;
        println!("[Simultaneous] => Resolveing for peer01 and peer02 tasks... DONE\n");
