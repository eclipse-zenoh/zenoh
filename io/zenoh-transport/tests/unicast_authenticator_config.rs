...
mod auth_config {
    ...
    fn keypair_pem() -> (String, String) {
        ...
        let primes = vec![
            BigUint::from_bytes_le(&[
                0x87, 0x9c, 0xbd, 0x9c, 0xbf, 0xd5, 0xb7, 0xc2, 0x73, 0x16, 0x44, 0x3f, 0x67,
                0x90, 0xaa, 0xab, 0xfe, 0x20, 0xac, 0x7d, 0xe9, 0xc4, 0xb9, 0xfb, 0x12, 0xab,
                0x09, 0x35, 0xec, 0xf5, 0x9f, 0xe1,
            ]),
            BigUint::from_bytes_le(&[
                0xf7, 0xa2, 0xc1, 0x81, 0x63, 0xe1, 0x1c, 0x39, 0xe4, 0x7b, 0xbf, 0x56, 0xd5,
                0x35, 0xc3, 0xd1, 0x11, 0xf9, 0x1f, 0x42, 0x4e, 0x3e, 0xe7, 0xc9, 0xa2, 0x3c,
                0x98, 0x08, 0xaa, 0xf9, 0x6b, 0xdf,
            ]),
        ];
    ...
    async fn auth_config_known_keys_file() {
        ...
        let bad_path = write_tmp_file("zenoh-test-cfg-known-bad.pem", "not valid pem content\n-----END RSA PUBLIC KEY-----\n");
        let mut config = PubKeyConf::default();
        config.set_public_key_pem(Some(pub_pem.clone())).unwrap();
        config.set_private_key_pem(Some(pri_pem.clone())).unwrap();
        config
