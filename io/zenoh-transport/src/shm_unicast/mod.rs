...
use transport::ShmTransportUnicastInner;
...
/// [`ShmTransportUnicast`] is the transport handler returned
/// when opening a new unicast transport for shm
#[derive(Clone)]
pub struct ShmTransportUnicast(Weak<ShmTransportUnicastInner>);

impl ShmTransportUnicast {
    #[inline(always)]
    pub(super) fn get_inner(&self) -> ZResult<Arc<ShmTransportUnicastInner>> {
        self.0
            .upgrade()
            .ok_or_else(|| zerror!("SHM transport unicast closed").into())
    }
    ...
    pub fn get_peer(&self) -> ZResult<TransportPeer> {
        ...
        let tp = TransportPeer {
            ...
            links: transport
                .get_link()
                .map_or(vec![], |val| vec![val.clone().into()]),
...
impl From<&Arc<ShmTransportUnicastInner>> for ShmTransportUnicast {
    fn from(s: &Arc<ShmTransportUnicastInner>) -> ShmTransportUnicast {
        ShmTransportUnicast(Arc::downgrade(s))
    }
}

impl Eq for ShmTransportUnicast {}

impl PartialEq for ShmTransportUnicast {
    fn eq(&self, other: &Self) -> bool {
        Weak::ptr_eq(&self.0, &other.0)
    }
}

impl fmt::Debug for ShmTransportUnicast {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.get_inner() {
            Ok(transport) => {
                let is_shm = zcondfeat!("shared-memory", transport.is_shm(), false);
                f.debug_struct("SHM Transport Unicast")
